...
public class WhitelistedApp {
    private static final String TAG = WhitelistedApp.class.getSimpleName();

    private List<ApplicationInfo> blacklistedAppList;
    private Context context;
    private PackageManager pm;

    public WhitelistedApp(Context c) {
        context = c;
    ...
    public boolean isInList(String s){
        try {
            return blacklistedAppList.contains(pm.getApplicationInfo(s, 0));
        } catch (PackageManager.NameNotFoundException e) {
            Log.e(TAG, "incorrect app");
        }
        return false;
    }
}
